...
class FloorAwareRoomSegmenter:
    ...
    def flush_pending_floor_segments(
        ...
        floor_ids = self.floor_manager._ordered_floor_ids()
        floors = []
    ...
    def get_vector_map_data(
        ...
        floors = self.floor_manager.export_floors()
        ...
        floor_lookup = {item["floor_id"]: dict(item) for item in floors}
        ...
        vector_data["vertical_transitions"] = self._build_vertical_transitions(room_records)
    ...
    def _build_floor_debug_summary(self, vector_map: Optional[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        floors = list(vector_map.get("floors") or self.floor_manager.export_floors())
        rooms = list(vector_map.get("rooms") or [])
        objects = list(vector_map.get("objects") or [])
        anchors = list(vector_map.get("anchors") or [])
        history = list(vector_map.get("frame_floor_assignments") or self.floor_manager.export_assignment_history())
        room_counts = {}
        object_counts = {}
        anchor_counts = {}
        for room in rooms:
    ...
    def _build_floor_diagnostics_markdown(self, report: Dict[str, Any]) -> str:
        ...
        for floor in report.get("per_floor", []):
            lines.append(
                "- "
                f"{floor.get('floor_id')} idx={floor.get('floor_index')} z=[{floor.get('z_min')}, {floor.get('z_max')}] center={floor.get('z_center')} "
                f"support={floor.get('support_count')} stable={floor.get('stable_frame_count')} transition={floor.get('transition_frame_count')} "
                f"stable_chunks={floor.get('stable_keyframe_chunk_count')} seg_triggers={floor.get('segmentation_trigger_count')} "
        ...
        for event in report.get("floor_hypothesis_events", []):
            lines.append(
                "- "
                f"frame {event.get('frame_idx')}: {event.get('event')} floor={event.get('floor_id')} candidate_z={event.get('candidate_z')} "
        ...
        if not finalization:
            ...
            for floor in finalization.get("floors", []):
                lines.append(
                    "- "
                    f"{floor.get('floor_id')}: pending_before={floor.get('pending_chunk_count_before')} segmented={floor.get('segmented')} reason={floor.get('reason')}"
                )
        ...
        if highest_floor:
            ...
            lines.append(
                "- "
                f"{highest_floor.get('floor_id')}: first_seen={highest_floor.get('first_seen_frame')} last_seen={highest_floor.get('last_seen_frame')} "
    ...
    def _build_vertical_transitions(self, room_records: Sequence[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        for idx in range(len(stable_segments) - 1):
            ...
            if left["floor_id"] == right["floor_id"]:
                continue
            left_room = _best_room_for_pose(left["items"][-1]["position_xy"], room_list, floor_id=left["floor_id"])
            right_room = _best_room_for_pose(right["items"][0]["position_xy"], room_list, floor_id=right["floor_id"])
            if left_room is None or right_room is None:
                continue
            confidence = min(
                float(left["items"][-1].get("confidence", 0.0)),
                float(right["items"][0].get("confidence", 0.0)),
            )
            vertical_transitions.append(
                {
                    "transition_id": f"vt_{len(vertical_transitions) + 1}",
                    "type": "vertical_transition",
                    "from_room_id": int(left_room["id"]),
                    "to_room_id": int(right_room["id"]),
                    "from_floor_id": str(left["floor_id"]),
                    "to_floor_id": str(right["floor_id"]),
                    "frame_start": int(left["frame_end"]),
                    "frame_end": int(right["frame_start"]),
                    "confidence": round(float(max(0.35, confidence)), 3),
                    "status": "supported",
                    "notes": "Cross-floor traversal inferred from stable floor-assignment change.",
                }
            )
