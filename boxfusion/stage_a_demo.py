...
from boxfusion.room_commit_diagnosis import (
    ...
    load_scene_artifacts as load_room_commit_scene_artifacts,
    render_room_commit_diagnosis_markdown,
    write_json as write_room_commit_diagnosis_json,
    write_markdown as write_room_commit_diagnosis_markdown,
)
...
def _floor_display_lookup(vector_map: Optional[Dict[str, Any]]) -> Dict[str, Dict[str, Any]]:
    vector_map = dict(vector_map or {})
    return {
        str(floor.get("floor_id")): dict(floor)
        for floor in vector_map.get("floors", [])
        if floor.get("floor_id") is not None
    }


def _display_floor_label(
    floor_id: Optional[str],
    vector_map: Optional[Dict[str, Any]] = None,
    floor_lookup: Optional[Dict[str, Dict[str, Any]]] = None,
) -> str:
    lookup = floor_lookup if floor_lookup is not None else _floor_display_lookup(vector_map)
    floor_meta = dict(lookup.get(str(floor_id)) or {}) if floor_id is not None else {}
    return canonical_display_floor_label(
        floor_id,
        floor_meta.get("display_floor_id"),
        floor_lookup=lookup,
...
def _transition_display_id(transition: Dict[str, Any], index: int) -> str:
    raw = transition.get("transition_id")
    if raw is None:
        return f"VT{int(index)}"
    text = str(raw).strip()
    if text.lower().startswith("vt_") and text.split("_", 1)[1].isdigit():
        return f"VT{text.split('_', 1)[1]}"
    return text.upper()


def _vertical_transition_lines(vector_map: Optional[Dict[str, Any]], limit: int = 4) -> List[str]:
    vector_map = dict(vector_map or {})
    summary = dict(vector_map.get("vertical_transition_summary") or {})
    transitions = list(summary.get("transitions") or vector_map.get("vertical_transitions", []))
    if not transitions:
        return ["No explicit vertical transitions exported."]
    lines: List[str] = []
    for idx, item in enumerate(transitions[: max(1, int(limit))], start=1):
        tid = _transition_display_id(item, idx)
        from_floor = item.get("from_display_floor_id", item.get("from_floor_id"))
        to_floor = item.get("to_display_floor_id", item.get("to_floor_id"))
        from_room = item.get("from_room_label")
        if from_room is None and item.get("from_room_id") is not None:
            from_room = f"R{int(item['from_room_id'])}"
        to_room = item.get("to_room_label")
        if to_room is None and item.get("to_room_id") is not None:
            to_room = f"R{int(item['to_room_id'])}"
        connector_label = item.get("connector_label", "unknown_vertical_connector")
        frame_start = item.get("transition_frame_start", item.get("frame_start"))
        frame_end = item.get("transition_frame_end", item.get("frame_end"))
        confidence = item.get("confidence")
        lines.append(
            f"{tid}: {from_floor}:{from_room or '?'} -> {to_floor}:{to_room or '?'} | {connector_label} | f{frame_start}-{frame_end} | c={confidence}"
        )
    return lines


def _segmentation_fallback_lines(vector_map: Optional[Dict[str, Any]], limit: int = 4) -> List[str]:
    vector_map = dict(vector_map or {})
    diagnostics = dict(vector_map.get("room_segmentation_diagnostics") or {})
    if not diagnostics:
        return ["No room-segmentation diagnostics exported."]
    lines = [
        f"Runs={diagnostics.get('run_count', 0)}; successful={diagnostics.get('successful_run_count', 0)}; fallback runs={diagnostics.get('fallback_run_count', 0)}; modes={diagnostics.get('fallback_counts', {})}"
    ]
    per_floor = sorted(
        diagnostics.get("per_floor", []),
        key=lambda item: (int(item.get("display_order", 10**6) or 10**6), str(item.get("display_floor_id", item.get("floor_id", "")))),
    )
    for floor in per_floor[: max(1, int(limit))]:
        display_floor_id = floor.get("display_floor_id", floor.get("floor_id"))
        lines.append(
            f"{display_floor_id}: runs={floor.get('run_count', 0)}; fallback runs={floor.get('fallback_run_count', 0)}; fallback counts={floor.get('fallback_counts', {})}"
        )
    return lines
...
class ClosedLoopDemoRecorder:
    def __init__(
        ...
        self.room_scoped_runtime = RoomScopedRuntimeManager(sequence_id=self.sequence_id)
    ...
    def record_snapshot(
        ...
        self.room_scoped_runtime.observe(
            frame_idx=int(frame_idx),
            timestamp=float(timestamp),
            current_room_id=current_room_id,
            vector_map=vector_map,
            export_profile=export_profile,
            lifecycle_manager=self.online_topology_lifecycle,
        )
    ...
    def record_frame(
        ...
        self.room_scoped_runtime.observe_tracking(
            frame_idx=int(frame_idx),
            timestamp=float(timestamp),
            current_room_id=current_room_id,
        )
    ...
    def finalize(
        ...
        vertical_transition_summary = dict(final_vector_map.get("vertical_transition_summary") or {})
        room_segmentation_diagnostics = dict(final_vector_map.get("room_segmentation_diagnostics") or {})
        ...
        room_scoped_runtime_state_json = self.log_dir / "room_scoped_runtime_state_v0_1.json"
        committed_room_world_model_json = self.log_dir / "committed_room_world_model_v0_1.json"
        public_world_snapshot_json = self.log_dir / "committed_room_world_snapshot_v0_1.json"
        ...
        room_commit_diagnosis_export_error = None
        room_topology = None
        full_public_topology_payload = None
        ...
        try:
            ...
            full_public_topology_payload = room_topology.to_dict(include_query_examples=False)
            room_scoped_export_result = self.room_scoped_runtime.export_artifacts(
        ...
        with open(vertical_transition_evidence_json, "w", encoding="utf-8") as f:
            json.dump(
                {
                    "sequence_id": self.sequence_id,
                    "summary": vertical_transition_summary,
                    "transitions": final_vector_map.get("vertical_transitions", []),
                },
                f,
                indent=2,
            )
        ...
        summary_payload.update(
            {
                ...
                "room_commit_diagnosis_export_error": room_commit_diagnosis_export_error,
            }
        )
    ...
    def _build_report(
        ...
        room_segmentation_diagnostics = dict(vector_map.get("room_segmentation_diagnostics") or {})
        transition_lines = _vertical_transition_lines(vector_map, limit=6)
        fallback_lines = _segmentation_fallback_lines(vector_map, limit=6)
        ...
        lines = [
            ...
            f"- Final vertical transitions: {int(vertical_transition_summary.get('count', len(vector_map.get('vertical_transitions', []))))}",
            f"- Segmentation fallback runs: {int(room_segmentation_diagnostics.get('fallback_run_count', 0))}",
            ...
            (
                "- Right panel uses teacher-facing multi-floor BEV small multiples, ordered from the lowest physical floor upward, and holds the latest real semantic snapshot until the next exported refresh instead of fabricating intermediate states."
                if full_rgb_replay
                else "- Right panel shows teacher-facing multi-floor BEV small multiples with shared XY scale, room ids, room-to-room adjacency edges, doorway/open-passage markers, and explicit vertical-transition annotations."
            ),
            (
        ...
        lines.extend(["## Cross-Floor Structure", ""])
        lines.extend([f"- {line}" for line in transition_lines])
        lines.extend(["", "## Segmentation Fallback Diagnostics", ""])
        lines.extend([f"- {line}" for line in fallback_lines])
        lines.extend(["", "## Strongest Revisit Examples", ""])
    ...
    def _render_bev(
        ...
        if map_snapshot is None and isinstance(frame_record, SnapshotRecord):
            map_snapshot = frame_record
        vector_map = {} if map_snapshot is None else (map_snapshot.vector_map or {})
        floor_lookup = _floor_display_lookup(vector_map)
        floors = sorted(
            list(vector_map.get("floors", [])),
            key=lambda item: (
                int(item.get("display_order", 10**6) or 10**6),
                float(item.get("z_center", 0.0) or 0.0),
            ),
        )
        if not floors:
        ...
        panel_transition_labels: Dict[str, List[str]] = {}
        for idx, transition in enumerate(vertical_transitions, start=1):
            transition_label = _transition_display_id(transition, idx)
            for floor_key in (transition.get("from_floor_id"), transition.get("to_floor_id")):
                if floor_key is None:
                    continue
                panel_transition_labels.setdefault(str(floor_key), []).append(transition_label)
        ...
        for index, floor in enumerate(floors):
            ...
            display_floor_id = str(floor.get("display_floor_id", floor_id))
            title = (
                f"{display_floor_id} | R/O/A "
                f"{summary.get('exported_room_count', 0)}/{summary.get('exported_object_count', 0)}/{summary.get('exported_anchor_count', 0)}"
            )
            subtitle = f"internal {floor_id} | z {float(floor.get('z_center', 0.0) or 0.0):.2f}m"
            cv2.putText(canvas, title[:52], (x0 + 10, y0 + 22), cv2.FONT_HERSHEY_SIMPLEX, 0.48, (45, 45, 45), 1, cv2.LINE_AA)
            cv2.putText(canvas, subtitle[:52], (x0 + 10, y0 + 40), cv2.FONT_HERSHEY_SIMPLEX, 0.38, (95, 95, 95), 1, cv2.LINE_AA)
            if is_active:
                cv2.putText(canvas, "ACTIVE", (x0 + panel_width - 74, y0 + 22), cv2.FONT_HERSHEY_SIMPLEX, 0.48, (24, 102, 196), 2, cv2.LINE_AA)
            vt_tags = panel_transition_labels.get(floor_id, [])
            if vt_tags:
                cv2.putText(canvas, f"links: {' '.join(vt_tags[:4])}"[:34], (x0 + 10, y0 + panel_height - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.36, (120, 98, 44), 1, cv2.LINE_AA)
        ...
        for idx, transition in enumerate(vertical_transitions, start=1):
            transition_label = _transition_display_id(transition, idx)
            raw_from_room_id = transition.get("from_room_id")
            raw_to_room_id = transition.get("to_room_id")
            from_room_id = int(raw_from_room_id) if raw_from_room_id is not None else -1
            to_room_id = int(raw_to_room_id) if raw_to_room_id is not None else -1
            source_pt = room_centers_canvas.get(from_room_id) if from_room_id >= 0 else None
            target_pt = room_centers_canvas.get(to_room_id) if to_room_id >= 0 else None
            if source_pt is None:
                from_floor_id = transition.get("from_floor_id")
                from_position = transition.get("from_position_xy")
                if from_floor_id is not None and from_position is not None and len(from_position) >= 2 and str(from_floor_id) in panel_rects:
                    source_pt = to_panel(from_position, panel_rects[str(from_floor_id)])
            if target_pt is None:
                to_floor_id = transition.get("to_floor_id")
                to_position = transition.get("to_position_xy")
                if to_floor_id is not None and to_position is not None and len(to_position) >= 2 and str(to_floor_id) in panel_rects:
                    target_pt = to_panel(to_position, panel_rects[str(to_floor_id)])
            ...
            cv2.putText(canvas, transition_label[:6], (source_pt[0] - 12, source_pt[1] - 14), cv2.FONT_HERSHEY_SIMPLEX, 0.4, color, 1, cv2.LINE_AA)
            cv2.putText(canvas, transition_label[:6], (target_pt[0] - 12, target_pt[1] - 14), cv2.FONT_HERSHEY_SIMPLEX, 0.4, color, 1, cv2.LINE_AA)
            cv2.line(canvas, source_pt, target_pt, color, 1, cv2.LINE_AA)

        legend = [
            "Panels are ordered from lowest to highest teacher-facing floor id",
        ...
        transition_notes = _vertical_transition_lines(vector_map, limit=3)
        legend.extend(transition_notes)
        if self.full_rgb_replay:
            legend.append("Panels hold between real semantic refreshes; the active border shows the current pose floor")
        _draw_text_block(canvas, legend, (18, height - 134), font_scale=0.46)
        return canvas
