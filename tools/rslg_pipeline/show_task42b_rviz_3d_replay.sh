...
/usr/bin/python3 "${PUBLISHER}" --markers-json "${MARKERS_JSON}" --frames-json "${FRAMES_JSON}" --loop --rate "${RSLG_TASK42B_REPLAY_RATE:-12}" --speed "${RSLG_TASK42B_REPLAY_SPEED:-1.0}" &
PUBLISHER_PID="$!"

echo "Task42b 3D replay publisher started on /task42b_3d_demo_markers (pid ${PUBLISHER_PID})."
echo "RViz Map display is not required. Gazebo, Nav2, map_server, and route executors are not launched."
...
if command -v rviz2 >/dev/null 2>&1 && [[ -n "${DISPLAY:-}" ]]; then
  ...
  echo "Press Ctrl+C to stop the task42b publisher."
  wait "${PUBLISHER_PID}"
fi
