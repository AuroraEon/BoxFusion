#!/usr/bin/env python3
"""Run the task39 canonical cross-floor room route in controlled simulation."""
...
TASK_NAME = "task39_authorized_runtime_adapter_validation_and_cross_floor_room_demo"
CANONICAL_ROOT = (
...
PROFILE = LAYER4 / "runtime_configs/task39_lightweight_runtime_profile_v0_1.json"
RUNTIME_WORLD = (
    REPO_ROOT
    / "tools/rslg_pipeline/runtime_assets/task39_empty_runtime_support_world.sdf"
)
FLOOR_BINDINGS = LAYER4 / "runtime_configs/task39_floor_segment_bindings_v0_1.json"
HANDOFF_PLAN = LAYER4 / "runtime_configs/task39_topological_handoff_plan_v0_1.json"
LOG_ROOT = LAYER4 / "logs"
STDIO_ROOT = LOG_ROOT / "runtime_stdout_stderr"
TRAJECTORY_PATH = LAYER4 / "trajectories/executed_trajectory_v0_1.json"
TRAJECTORY_SUMMARY_PATH = (
    LAYER4 / "trajectories/executed_trajectory_summary_v0_1.json"
)
RUNTIME_RESULT_PATH = LAYER4 / "validation_reports/task39_runtime_result_v0_1.json"
...
def validate_inputs() -> tuple[dict[str, Any], dict[str, Any], dict[str, Any]]:
    ...
    route = read_json(RUNTIME_INPUT)
    package = read_json(RUNTIME_PACKAGE)
    maps = read_json(MAP_INPUTS)
    if route.get("route_id") != "cross_floor_room":
        raise ValueError("task39 accepts only route_id=cross_floor_room")
    if route.get("selected_profile") != "conservative_canonical":
        raise ValueError("task39 accepts only profile=conservative_canonical")
    ...
    if package.get("object_route_selected"):
        raise ValueError("object route must not be selected")
...
def binding_payloads(
    ...
    floor_bindings = {
        "schema_name": "rslg_task39_floor_segment_bindings",
        "schema_version": "0.1",
    ...
    handoff = {
        "schema_name": "rslg_task39_topological_handoff_plan",
        "schema_version": "0.1",
        "project_name": "RSLG-SLAM",
        "artifact_layer": "Layer 4: Runtime Validation Layer",
        "connector_id": "vt_1",
    ...
    profile = {
        "schema_name": "rslg_task39_lightweight_runtime_profile",
        "schema_version": "0.1",
        "project_name": "RSLG-SLAM",
        "scene_id": SCENE_ID,
        "artifact_layer": "Layer 4: Runtime Validation Layer",
...
def prepare_bindings() -> dict[str, Any]:
    ...
    return {
        ...
        "selected_route_runtime_input": rel(RUNTIME_INPUT),
        ...
        "package_runtime_authorization_before_task39": package.get(
...
def execute_runtime(args: argparse.Namespace) -> dict[str, Any]:
    ...
    if os.environ.get("RSLG_TASK39_ALLOW_RUNTIME") != "1":
        raise PermissionError("RSLG_TASK39_ALLOW_RUNTIME=1 is required")
    ...
    launch_log = STDIO_ROOT / "task39_gazebo_bringup.log"
    map_log = STDIO_ROOT / "task39_map_server.log"
    adapter_log = STDIO_ROOT / "task39_runtime_adapter.log"
    command_records: list[dict[str, Any]] = []
    trajectory: list[dict[str, Any]] = []
    started = now_iso()
    result: dict[str, Any] = {
        "schema_name": "rslg_task39_runtime_result",
        "schema_version": "0.1",
        "project_name": "RSLG-SLAM",
        "scene_id": SCENE_ID,
        "artifact_layer": "Layer 4: Runtime Validation Layer",
    ...
    try:
        ...
        with map_log.open("w", encoding="utf-8") as map_handle:
            ...
            class RuntimeNode(Node):
                def __init__(self) -> None:
                    super().__init__("rslg_task39_cross_floor_runtime_adapter")
                ...
                def publish_overlays(self, active_floor: str, force: bool = False) -> None:
                    ...
                    route_markers.markers = [
                        self.line_marker(
                            1, "task39/floor_1_route", floor1, self.color(0.1, 0.55, 1.0), 0.12
                        ),
                        self.line_marker(
                            2, "task39/floor_2_route", floor2, self.color(0.7, 0.25, 0.95), 0.18
                        ),
                    ]
                    if trajectory:
                    ...
                    anchors.markers = [
                        self.sphere_marker(
                            1, "task39/start", floor1[0], self.color(0.1, 0.9, 0.2), 0.35
                        ),
                        self.sphere_marker(
                            2, "task39/floor_1_terminal", floor1[-1], self.color(1.0, 0.65, 0.0), 0.35
                        ),
                        self.sphere_marker(
                            3, "task39/floor_2_start", floor2[0], self.color(1.0, 0.65, 0.0), 0.35
                        ),
                        self.sphere_marker(
                            4, "task39/final_room", floor2[-1], self.color(0.95, 0.1, 0.15), 0.35
                        ),
                    ]
                    ...
                    connector.markers = [
                        self.line_marker(
                            1,
                            "task39/vt_1_centerline_e001_topological_handoff",
                            [floor1[-1], floor2[0]],
            ...
            result["runtime_components_launched"].extend(
                [
                    "task39 direct velocity route executor",
                    "task39 MarkerArray overlay publisher",
                    "task39 TF/odom trajectory logger",
                ]
            )
            ...
            result["final_room_status"] = "room_14_reached_within_runtime_tolerance"
            node.publish_overlays("floor_2", force=True)
            time.sleep(1.0)
            node.stop()
            node.destroy_node()
        ...
        write_json(
            TRAJECTORY_PATH,
            {
                "schema_name": "rslg_task39_executed_trajectory",
                "schema_version": "0.1",
                "project_name": "RSLG-SLAM",
                "scene_id": SCENE_ID,
                "artifact_layer": "Layer 4: Runtime Validation Layer",
...
def summarize_trajectory(
    ...
    return {
        "schema_name": "rslg_task39_executed_trajectory_summary",
        "schema_version": "0.1",
        "project_name": "RSLG-SLAM",
        "scene_id": SCENE_ID,
        "artifact_layer": "Layer 4: Runtime Validation Layer",
