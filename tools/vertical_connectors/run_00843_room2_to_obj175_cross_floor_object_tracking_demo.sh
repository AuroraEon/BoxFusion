...
PY="/usr/bin/python3"
BUILDER="${SCRIPT_DIR}/build_cross_floor_object_approach_route.py"
PLAYER="${SCRIPT_DIR}/cross_floor_object_tracking_player.py"
OUT_DIR="${REPO_ROOT}/stage_outputs/stage1_generalization/00843-DYehNKdT76V/tasks/task24i_cross_floor_object_level_tracking_smoke"
PLANNED_ROUTE="${OUT_DIR}/planned_object_level_3d_route_v0_1.json"
...
URDF="${RUNTIME_DIR}/rslg_cross_floor_object_quadruped_proxy_tracking_visual_only_fixed.urdf"
WORLD="${RUNTIME_DIR}/task24i_empty_world_with_state_plugin.sdf"
SOURCE_WORLD="/usr/share/gazebo-11/worlds/empty.world"
RVIZ_CONFIG="${SCRIPT_DIR}/rviz_room2_to_obj175_cross_floor_object_tracking.rviz"
RVIZ_MODE="auto"
GAZEBO_MODE="auto"
KEEP_OPEN="0"
HOLD_FINAL_SEC="8.0"
...
GZCLIENT_PID=""
RVIZ_PID=""
ORIGINAL_COMMAND="tools/vertical_connectors/run_00843_room2_to_obj175_cross_floor_object_tracking_demo.sh"
for arg in "$@"; do
...
usage() {
  cat <<'EOF'
Usage:
  tools/vertical_connectors/run_00843_room2_to_obj175_cross_floor_object_tracking_demo.sh [options]
...
Options:
  ...
  --manual-demo      Human GUI mode: realtime scale 2.0, hold final 30s, keep open.
  --demo-speed MODE  Demo preset: slow, normal, or fast.
  --rebuild-route    Rebuild the task24i object route before running.
...
echo "RSLG-SLAM task24i cross-floor object-level tracking smoke"
echo "Project path: ${REPO_ROOT}"
echo "Object query: curtain in room_14 on floor_2"
echo "Object target: obj_175 / curtain"
echo "Approach candidate: generated_ring_037"
echo "Planned object route: ${PLANNED_ROUTE}"
...
echo "object_centroid_navigation_used=false"
...
if [[ "${REBUILD_ROUTE}" == "1" || ! -f "${PLANNED_ROUTE}" ]]; then
  "${PY}" "${BUILDER}" --output-dir "${OUT_DIR}"
else
  echo "Reusing existing task24i object route. Pass --rebuild-route to regenerate it."
fi
...
if [[ "${GAZEBO_MODE}" != "off" ]]; then
  if ! wait_for_service "/spawn_entity" 45 && ! wait_for_service "/gazebo/spawn_entity" 5; then
    ...
    echo "Spawning or reusing visual-only CHAMP-shaped object tracking proxy."
...
PLAYER_ARGS=(
  ...
  --entity-name "${ENTITY_NAME}"
  --frame-id world
  --base-frame rslg_cross_floor_object_tracking_base_link
...
echo "task24i outputs written under: ${OUT_DIR}"
echo "Report: ${OUT_DIR}/task24i_report.md"
