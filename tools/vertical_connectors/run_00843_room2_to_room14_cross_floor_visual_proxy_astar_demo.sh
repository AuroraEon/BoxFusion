...
PY="/usr/bin/python3"
BUILDER="${SCRIPT_DIR}/build_occupancy_aware_cross_floor_visual_route.py"
...
OUT_DIR="${REPO_ROOT}/stage_outputs/stage1_generalization/00843-DYehNKdT76V/tasks/task24g2_occupancy_aware_cross_floor_visual_proxy_traversal"
PLANNED_ROUTE="${OUT_DIR}/planned_occupancy_aware_3d_route_v0_1.json"
...
WORLD="${RUNTIME_DIR}/task24g2_empty_world_with_state_plugin.sdf"
SOURCE_WORLD="/usr/share/gazebo-11/worlds/empty.world"
RVIZ_CONFIG="${SCRIPT_DIR}/rviz_room2_to_room14_cross_floor_visual_proxy_astar.rviz"
RVIZ_MODE="auto"
GAZEBO_MODE="auto"
KEEP_OPEN="0"
REBUILD_ROUTE="0"
...
GZCLIENT_PID=""
RVIZ_PID=""
ORIGINAL_COMMAND="tools/vertical_connectors/run_00843_room2_to_room14_cross_floor_visual_proxy_astar_demo.sh"
for arg in "$@"; do
...
usage() {
  cat <<'EOF'
Usage:
  tools/vertical_connectors/run_00843_room2_to_room14_cross_floor_visual_proxy_astar_demo.sh [options]

Options:
  --rebuild-route    Rebuild planned_occupancy_aware_3d_route_v0_1.json before playback.
...
echo "RSLG-SLAM task24g2 occupancy-aware cross-floor visual proxy traversal demo"
echo "Project path: ${REPO_ROOT}"
echo "Route contract: ${ROUTE_CONTRACT}"
echo "Planned occupancy-aware route: ${PLANNED_ROUTE}"
...
echo "occupancy_aware_same_floor_visual_playback=true"
...
if [[ "${REBUILD_ROUTE}" == "1" || ! -f "${PLANNED_ROUTE}" ]]; then
  echo "Building occupancy-aware planned route."
  "${PY}" "${BUILDER}" \
    --route-contract "${ROUTE_CONTRACT}" \
    --output-dir "${OUT_DIR}"
else
  echo "Reusing existing occupancy-aware planned route. Pass --rebuild-route to regenerate it."
fi
...
read -r SPAWN_X SPAWN_Y SPAWN_Z SPAWN_YAW < <("${PY}" "${PLAYER}" --route-contract "${ROUTE_CONTRACT}" --planned-route-json "${PLANNED_ROUTE}" --print-start-pose)
...
echo "task24g2 outputs written under: ${OUT_DIR}"
echo "Report: ${OUT_DIR}/task24g2_report.md"
